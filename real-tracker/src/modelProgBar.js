...
const ProgBar = () => {
  ...
  useEffect(() => {
    async function fetchMaxPercentageOAN() {
      try {
        const response = await axios.get('http://localhost:5000/maxPercentageOAN'); // Use Axios for GET request
  ...
  useEffect(() => {
    async function fetchPlacedBlocks() {
      try {
        const response = await axios.get('http://localhost:5000/placedBlocks'); // Use Axios for GET request
